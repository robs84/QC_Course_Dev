...
Introduction to State Visualization will have several small steps with the goal of understanding what some fundamental one-qubit gates do when operating on a qubit of a given state.  This understanding will come from visualizing rotations about the Bloch sphere.
...
- Okay, so we have seen rotations from the |0> state to the |1> state and the |+> state (+x-direction).  Now, using the Pauli, Hadamard, and S gates rotate the along the Bloch sphere to align the state vector with |-> (-x-direction), |R> (+y-direction), and |L> (-y-direction).
